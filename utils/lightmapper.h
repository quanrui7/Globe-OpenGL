...
struct lm_context
{
	...
	struct
	{
		unsigned int size; // 半球大小
		float zNear, zFar; // 摄像机的近裁剪面和远裁剪面
		float cameraToSurfaceDistanceModifier; // 摄像机到表面的距离修正因子
		struct { float r, g, b; } clearColor; // 清除颜色的红、绿、蓝分量

		unsigned int fbHemiCountX; // 帧缓冲中半球在X轴的数量
		unsigned int fbHemiCountY; // 帧缓冲中半球在Y轴的数量
		unsigned int fbHemiIndex; // 当前处理的半球索引
		lm_ivec2 *fbHemiToLightmapLocation; // 半球到光照贴图位置的映射关系
		GLuint fbTexture[2]; // 双缓冲帧缓冲纹理对象
		GLuint fb[2]; // 双缓冲帧缓冲对象
		GLuint fbDepth; // 深度缓冲对象
		GLuint vao; // 顶点数组对象
		struct
		{
			GLuint programID; // 第一遍渲染时使用的着色器程序ID
			GLuint hemispheresTextureID; // 第一遍渲染时使用的半球纹理ID
			GLuint weightsTextureID; // 第一遍渲染时使用的权重纹理ID
			GLuint weightsTexture; // 权重纹理对象
		} firstPass; // 第一遍渲染相关设置
		struct
		{
			GLuint programID; // 下采样时使用的着色器程序ID
			GLuint hemispheresTextureID; // 下采样时使用的半球纹理ID
		} downsamplePass; // 下采样渲染相关设置
		struct
		{
			GLuint texture; // 存储光照贴图的纹理对象
			lm_ivec2 writePosition; // 当前写入光照贴图的位置
			lm_ivec2 *toLightmapLocation; // 指向光照贴图位置的指针
		} storage; // 存储相关设置
	} hemisphere; // 半球结构体，用于管理光照贴图生成过程中的各类资源和状态
